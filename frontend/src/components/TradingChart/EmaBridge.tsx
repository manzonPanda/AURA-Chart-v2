 ...
 *   (WS truth REPLACES the forming bucket's close) → calculateEMA() → line.
 * The rAF-animated close is NEVER an input, so background-tab throttling and
 * the cosmetic glide cannot skew EMA values; on tab return the next WS frame
 * (a full server snapshot) re-anchors both the candle and the EMA.
 *
...
export function EmaBridge({ bars, liveCandle, bucketSec, settings }: Props) {
  ...
  useEffect(() => {
    ...
    for (const slot of EMA_SLOTS) {
      ...
      const points = calculateEMA(closes, cfg.period);
      if (points.length === 0) {
